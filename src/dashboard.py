...
portfolio risk metrics, and asset/sector allocations for multiple tickers.
...
- **Sidebar controls:**
  ...
  - Select historical data period and price interval. **Interval options are dynamically filtered based on the selected period.**
...
- **Advanced risk & performance metrics:**
  - VaR (95%), CVaR (95%), Sharpe, Sortino, Calmar ratios computed from **portfolio value returns**.
  - **Max Drawdown computed from the portfolio's total market value over time.**
  - Asset Correlation Matrix table with gradient styling.
  - Win/Loss stats derived from portfolio returns.
...
- altair
- plotly
- pnl_calc (calculate_pnl)
- state_manager (init_state, update_state, [and interval/data editor helpers])
